    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-gray-800 rounded-lg p-4 md:p-6 mb-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                <div class="bg-gray-100 p-4 md:p-6 rounded-lg shadow flex items-center transition-all hover:shadow-md">
                    ...
                    <div class="w-full">
                        <h3 class="text-sm md:text-base font-semibold uppercase text-gray-700">Jumlah Unit per Produk</h3>
                        <select id="product-counts" class="w-full bg-white text-gray-900 text-sm md:text-base rounded-md py-1 px-2 focus:outline-none focus:ring-2 focus:ring-orange-500 border border-gray-300">
        ...
        <div class="bg-gray-800 p-4 md:p-6 mb-6 rounded-lg shadow-sm">
            <form id="search-form" action="{{ route('inventory.search') }}" method="GET" class="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full">
                <div class="relative flex-grow">
                    <input type="text" name="search" id="search-input" class="w-full bg-white text-gray-900 text-sm md:text-base rounded-lg py-2.5 px-4 pr-10 focus:outline-none focus:ring-2 focus:ring-orange-500 placeholder-gray-500 border border-gray-300" placeholder="Cari produk..." value="{{ $searchTerm ?? '' }}">
        ...
        <div class="bg-gray-800 p-4 md:p-6 mb-6 rounded-lg shadow-sm">
            <div class="flex flex-col md:flex-row gap-3">
                ...
                <a href="{{ route('inventory.history') }}" class="bg-gray-600 text-white font-medium text-sm md:text-base rounded-lg px-4 py-2.5 hover:bg-gray-700 transition-colors flex items-center w-full md:w-auto justify-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Riwayat Produk
                </a>
                <a href="{{ route('purchase_notes.index') }}" class="bg-gray-600 text-white font-medium text-sm md:text-base rounded-lg px-4 py-2.5 hover:bg-gray-700 transition-colors flex items-center w-full md:w-auto justify-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="shadow rounded-lg overflow-hidden hidden md:block bg-gray-800 p-4">
            <div class="rounded-lg overflow-hidden">
                ...
                <div class="grid grid-cols-10 gap-0 bg-orange-500 text-black font-medium">
                    ...
                    <div class="py-2 px-3 text-center">Produk</div>
                    <div class="py-2 px-3 text-center">Ukuran</div>
                    <div class="py-2 px-3 text-center">Warna</div>
                    <div class="py-2 px-3 text-center">Stok Sistem</div>
                    <div class="py-2 px-3 text-center">Stok Fisik</div>
                    <div class="py-2 px-3 text-center">Harga Jual</div>
                    <div class="py-2 px-3 text-center">Harga Diskon</div>
                    <div class="py-2 px-3 text-center">QR Code</div>
                    <div class="py-2 px-3 text-center">Actions</div>
                </div>
                ...
                <div class="mt-1" id="desktop-table-body">
                    ...
                    @forelse ($products->filter(fn($product) => $product->product_units_count > 0) ?? [] as $product)
                        ...
                        @if ($currentGroup !== $productName)
                            ...
                            @php $currentGroup = $productName; @endphp
                        @endif
            ...
            @forelse ($products->filter(fn($product) => $product->product_units_count > 0) ?? [] as $product)
                ...
                <div class="bg-white rounded-lg shadow p-4">
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=50x50&data={{ urlencode(route('inventory.show', $product->id)) }}" alt="QR Code" class="h-12 w-12" onerror="this.src='{{ asset('images/qr-placeholder.png') }}';">
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            data.products.forEach((product, index) => {
                ...
                desktopRow += `
                    <div class="grid grid-cols-10 gap-0 items-center ${index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}">
                        ...
                        <div class="p-3 text-center">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent('{{ route('inventory.show', ':id') }}'.replace(':id', product.id))}" alt="QR Code" class="h-12 w-12 mx-auto" onerror="this.src='{{ asset('images/qr-placeholder.png') }}';">
                        </div>
                        <div class="p-3">
                            <div class="flex justify-center space-x-3">
                                <a href="{{ route('inventory.edit', ':id') }}".replace(':id', product.id) class="text-blue-600 hover:text-blue-800" title="Edit">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <a href="{{ route('inventory.print_qr', ':id') }}".replace(':id', product.id) class="text-green-600 hover:text-green-800" title="Cetak QR">
                                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                    </svg>
                                </a>
                ...
                if (currentGroup !== productName) {
                    ...
                    mobileHtml += `<div class="bg-orange-500 text-black font-semibold py-2 px-3 rounded-t-lg uppercase-text">${productName}</div><div class="space-y-4">`;
                    currentGroup = productName;
                }
                ...
                mobileHtml += `
                    <div class="bg-white rounded-lg shadow p-4">
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=50x50&data=${encodeURIComponent('{{ route('inventory.show', ':id') }}'.replace(':id', product.id))}" alt="QR Code" class="h-12 w-12" onerror="this.src='{{ asset('images/qr-placeholder.png') }}';">
                        </div>
                        <div class="grid grid-cols-2 gap-2 mb-3 text-sm">
                            <div>
                                <div class="text-gray-500">Stok Sistem</div>
                        ...
                        <div class="flex justify-end space-x-3 pt-2 border-t border-gray-100">
                            <a href="{{ route('inventory.edit', ':id') }}".replace(':id', product.id) class="text-blue-600 hover:text-blue-800 flex items-center text-sm">
                                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                                Edit
                            </a>
                            <a href="{{ route('inventory.print_qr', ':id') }}".replace(':id', product.id) class="text-green-600 hover:text-green-800 flex items-center text-sm">
                                <svg class="h-4 w-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                </svg>
                                Print QR
    ...
    function updatePagination(pagination, searchKeyword, sizeKeyword, lowStock) {
        ...
        html += `<a href="#" class="pagination-link ${currentPage === 1 ? 'disabled' : ''}" data-page="${currentPage - 1}"><span>Previous</span></a>`;
        ...
        html += `<a href="#" class="pagination-link ${currentPage === totalPages ? 'disabled' : ''}" data-page="${currentPage + 1}"><span>Next</span></a>`;
        html += '</nav>';
