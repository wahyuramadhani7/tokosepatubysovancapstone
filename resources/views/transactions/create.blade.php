...
<body class="min-h-screen custom-scrollbar" x-data="transactionApp()" x-init="initialize()">
    ...
    <header class="fixed top-0 w-full header text-white z-50">
        <div class="container mx-auto px-4 py-3 flex justify-between items-center">
            ...
            <a href="{{ route('transactions.index') }}" class="p-2 bg-brand-dark-700 rounded-lg text-white hover:bg-brand-primary hover-scale" title="Kembali">
    ...
    <main class="container mx-auto px-4 py-10 max-w-6xl">
        <!-- Error Messages -->
        @if ($errors->any())
            <div class="mb-6 bg-red-900/20 border border-red-500/20 text-white p-4 rounded-lg fade-in flex items-center justify-between">
        ...
        <div class="mb-8 card rounded-lg p-6 fade-in">
            ...
            <p class="text-sm text-gray-400">Pilih unit produk dan lengkapi detail transaksi dengan mudah</p>
        </div>
        ...
        <div x-show="showPopup" class="modal-overlay" x-cloak x-transition:enter="popup-enter" @click.self="closePopup">
            <div class="card rounded-lg p-6 w-full max-w-md">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-base font-semibold text-white font-['Montserrat']" x-text="popupTitle"></h3>
                    <button type="button" @click="closePopup" class="text-gray-400 hover:text-brand-primary p-1 rounded-full hover:bg-brand-dark-700">
                        <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <form action="{{ route('transactions.store') }}" method="POST" @submit="validateForm($event)">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card rounded-lg p-6 fade-in">
                    ...
                    <div x-show="isScannerOpen" class="modal-overlay" x-cloak @click.self="closeScanner">
                        <div class="card rounded-lg p-6 w-full max-w-lg">
                            <div class="flex justify-between items-center mb-4">
                                <h3 class="text-base font-semibold text-white font-['Montserrat']">Scan QR Code Unit</h3>
                                <button type="button" @click="closeScanner" class="text-gray-400 hover:text-brand-primary p-1 rounded-full hover:bg-brand-dark-700">
                                    <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                </button>
                            ...
                            <template x-for="unit in searchResults" :key="unit.unit_code">
                                <li class="p-3 transition-colors cursor-pointer" @click="addToCart(unit)">
                                    <div class="flex justify-between items-center">
                                        <div>
                                            <p class="font-medium text-white text-sm" x-text="unit.product_name"></p>
                                        ...
                                        <div class="text-right">
                                            <p class="font-medium text-white text-sm" x-text="unit.discount_price ? formatRupiah(unit.discount_price) : formatRupiah(unit.selling_price)"></p>
                ...
                <div class="card rounded-lg p-6 fade-in">
                    ...
                    <!-- Cart -->
                    <h2 class="text-lg font-semibold text-white mb-4 flex items-center border-t border-brand-primary/20 pt-4">
                        <svg class="h-5 w-5 mr-2 text-brand-primary" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div x-show="cart.length === 0" class="text-center py-10 text-gray-400 border border-brand-primary/20 rounded-lg bg-brand-dark-700/20 mb-4">
                    ...
                    <div x-show="cart.length > 0" class="max-h-56 overflow-y-auto custom-scrollbar mb-4 space-y-3">
                        <ul class="space-y-2">
                            <template x-for="(item, index) in cart" :key="index">
                                <li class="border border-brand-primary/20 rounded-lg p-3 bg-brand-dark-700/20 relative fade-in">
                                    <button type="button" @click="removeItem(index)" class="absolute top-2 right-2 text-gray-400 hover:text-red-300">
                                        <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>
                                    <p class="font-medium text-white pr-6 text-sm" x-text="item.name"></p>
                                    <div class="flex items-center mt-1 space-x-2">
                                    ...
                                    <div class="flex justify-between items-center mt-3">
                                        <p class="font-medium text-white text-sm" x-text="item.discount_price ? formatRupiah(item.discount_price) : formatRupiah(item.selling_price)"></p>
                                    </div>
                    ...
                    <div class="border-t border-brand-primary/20 pt-4">
                        <h2 class="text-lg font-semibold text-white mb-3 font-['Montserrat']">Ringkasan Pembayaran</h2>
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <input type="number" name="discount_amount" x-model="discount" min="0" value="{{ old('discount_amount', 0) }}" class="w-20 text-right px-2 py-1 text-sm">
                                </div>
                            </div>
                            <div class="flex justify-between border-t border-brand-primary/20 pt-3 text-sm">
                                ...
                                <span class="font-bold text-base text-brand-primary" x-text="formatRupiah(calculateTotal())"></span>
                            </div>
                        </div>
